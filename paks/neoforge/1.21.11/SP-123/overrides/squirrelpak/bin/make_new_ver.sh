...
echo "   - Update bcc-common.json version via nano/vim"
echo " "
printf "%s " "Press enter to continue"
read ans
nano ./config/bcc-common.json
echo "     done"
echo " "
...
# Enable FM Editing
echo " "
echo "   - Enable Fancy Menu Editing"
sed -i '' "s/B:show_customization_overlay = '[^']*';/B:show_customization_overlay = 'true';/" config/fancymenu/options.txt
sed -i '' "s/B:modpack_mode = '[^']*';/B:modpack_mode = 'false';/" config/fancymenu/options.txt
echo " "
echo " ----------------------------------------------------------------------------"
